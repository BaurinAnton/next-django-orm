...
export const AddRow = ({ dataTable, actionTable, onAddRowSubmit }: TProps) => {
    ...
    const curentSubmitHandler = (changeDataRow: Omit<TAddDataRow, 'id'> | null) => {
    ...
    const changeInputHandler = (item: keyof TAddDataRow) => (e: ChangeEvent<HTMLInputElement>) => {
        setRowDataFormHandler({
            // ...rowDataForm,
            [`${item}`]: e.target.value
        } as TAddDataRow)
    ...
    return (
        ...
        >
            <div className={style.buttonAdding}>
                <button
                    className={`${style.button} ${actionCRUD === CRUDEnum.create
                        ? style.buttonActive
                        : ''}`}
                        ...
                        >
                            {
                                tableCell.map((item, id) => (
                                    (item != 'id') && (item != 'hire_date') && (item != 'create_date') && (
                                        <Flex
                                            flexDirection="column"
                                            alignItems="center"
                                            margin="0 1rem"
